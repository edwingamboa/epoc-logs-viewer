(function() {
  ...
  Promise.all([performanceMeasuresRequest, eventsRequest, userTraceRequest])
    .then(function(responses) {
      ...
      var relativeEngChanValsAndEvents = calculateRelativeEngChangeValuesAndEvents(pmMeasures, 1, 0);
      addTimeLineChart(
  ...
  function addTimeLineChart(data, containerId, eventsMarks, onrendered) {
    var chartProperties = {
      data: {
        rows: data,
        x: 'TimeStamp'
  ...
  function calculateRelativeEngChangeValuesAndEvents (pmList, engColumnIndex, timeColumnIndex) {
    var initialTime = pmList[1][timeColumnIndex];
    var initialEng = pmList[1][engColumnIndex];
    var currentEng;
    var currentTime;
    var relativeEngChange;
    var trend;
    var relEngChangeVals = [['TimeStamp', 'Relative Eng. Change']];
    var eventsLines = [];
    pmList.forEach(function(pm, i) {
      if (i > 1){
        ...
        if (initialTime) {
          ...
          trend = relativeEngChange > 0 ? 'Increase': 'Decrease';
          console.log(relativeEngChange, trend);
          relEngChangeVals.push([currentTime, relativeEngChange]);
          if ((currentTime.getMinutes() - initialTime.getMinutes()) === 1) {
