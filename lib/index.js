(function() {
  ...
  function calculateTrendDataOfPm (pmList, engColumnIndex, timeColumnIndex) {
    ...
    var data = [['TimeStamp', 'Relative Eng. Change', 'TrendPoint']];
    var eventsLines = [];
    pmList.slice(1).forEach(function(pm) {
      ...
      csv += currentTime.getTime() + ',' + parseFloat(currentEng) + '\n';
    ...
    relEngChangeVals.forEach(function (relChange) {
      var f = ss.linearRegressionLine(ss.linearRegression(relEngChangeVals));
      data.push([relChange[0], relChange[1], f(relChange[0])]);
    });
    return { 
