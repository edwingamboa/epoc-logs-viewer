(function() {
  ...
  Promise.all([performanceMeasuresRequest, eventsRequest, userTraceRequest])
    .then(function(responses) {
      ...
      var userTraceRegions = parseUserTraceAsGridLines(extractUserTraceEvents(responses[2].data));
      ...
      var eventsOfInterestGridLines = parseUserTraceAsGridLines(extractSegmentsFromTraceEvents(responses[2].data, linkValues, 2, 5, 30));
      addChart(
  ...
  function calculateTrendDataOfPm (pmList, engColumnIndex, timeColumnIndex) {
    var initialSegmentTime = pmList[1][timeColumnIndex];
    ...
    pmList.slice(1).forEach(function(pm, i) {
      ...
      if (elapsedMinutes(initialSegmentTime, currentTime) === 3) {
        // Trend Line
        addTrendPoints(relEngChangeVals, segmentInitIndex, i);
        // Update data for new Segment
        initialEng = currentEng;
        eventsLines.push({ value: currentTime });
        initialSegmentTime = currentTime;
        segmentInitIndex = i;
      }
    });
    // Last segment Trend Line
    addTrendPoints(relEngChangeVals, segmentInitIndex, relEngChangeVals.length -1);
  ...
  function addTrendPoints (data, segmentInitIndex, segmentFinalIndex) {
    var f = ss.linearRegressionLine(ss.linearRegression(data.slice(segmentInitIndex, segmentFinalIndex)));
  ...
  function elapsedSeconds (initTime, finishTime) {
    return Math.round((finishTime.getTime() - initTime.getTime())/1000)
  }
