...
(function () {
  ...
  const pmLogsInfo = new Map([
    ['TimeStamp', { col: 0 }],
    ['SCA_ENG', { col: 1, verbose: 'Engagement' }],
    ['SCA_VAL', { col: 6, verbose: 'Interest' }],
    ['SCA_MED', { col: 11, verbose: 'Relaxation' }],
    ['SCA_FRU', { col: 16, verbose: 'Stress' }],
    ['SCA_FOC', { col: 21, verbose: 'Focus' }],
    ['SCA_EXC', { col: 26, verbose: 'Excitement' }],
    ['SCA_LEX', { col: 31, verbose: 'Long-term excitement' }],
  ...
  function run (requests) {
    Promise.all(requests)
      .then(function (responses) {
        ...
        var segments = extractSegmentsFromTraceEvents(
          responses[1].data,
          traceLogsInfo.get('actionD1').values.link,
          traceLogsInfo.get('timestamp').col,
          traceLogsInfo.get('actionD1').col,
          DEFAULT_SEGMENT_DISTANCE
        );
        var relativeEngChanValsAndEvents = calculateTrendDataOfPm(pmMeasures, segments);
  ...
  function extractScaPerformanceMeasuresFromCSV (csv, desiredPms) {
    ...
    getLinesOfCSV(csv).forEach(function (line, i) {
      if (line !== '') {
        ...
        if (i > 0) {
          newColumns.push(
            dateBasedOnTimeStampMs(columns[pmLogsInfo.get('TimeStamp').col])
          );
        } else {
          newColumns.push(columns[pmLogsInfo.get('TimeStamp').col]);
        }
        desiredPms.forEach(function (pm) {
          newColumns.push(columns[pmLogsInfo.get(pm).col]);
        });
  ...
  function extractUserTraceEvents (csv) {
    ...
    getLinesOfCSV(csv).forEach(function (line, index) {
      if (line !== '' && index > 0) {
        var columns = getColumnsOfACsvLine(line, ';');
        dataRows.push({
          time: new Date(columns[traceLogsInfo.get('timestamp').col]),
          action: columns[traceLogsInfo.get('action').col],
          details: columns.slice(traceLogsInfo.get('action').col).join(' ')
        });
      }
    });
    return dataRows;
  ...
  function getVerboseOfPM (pmId) {
    return (
      pmId +
      (pmLogsInfo.has(pmId) && pmLogsInfo.get(pmId).hasOwnProperty('verbose')
        ? ', ' + pmLogsInfo.get(pmId).verbose
        : '')
    );
  }
  ...
  function calculateTrendDataOfPm (pmList, segments) {
    const timeColumnIndex = pmLogsInfo.get('TimeStamp').col;
    const engColumnIndex = pmLogsInfo.get('SCA_ENG').col;
    ...
    pmList.slice(1).forEach(function (pm) {
      ...
      relEngChangeVals.push([
        currentTime.getTime(),
        parseFloat(relativeEngagementChange(currentEng, initialEng))
      ]);
    });
